Q-Link : https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iii/description/
...
public:
 int helper(vector<int>&prices,int idx,int n,int buy,int cap,         vector<vector<vector<int>>>&dp){
        ...
        int profit=0;
        if(buy)profit=max(-1*prices[idx]+helper(prices,idx+1,n,0,cap,dp),helper(prices,idx+1,
        n,1,cap,dp));
